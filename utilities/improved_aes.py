...
# ---------------- Test Utilities ----------------
def assert_roundtrip(pt: str, k: int):
    ct = encrypt_sbox_xor(pt, k)
    rt = decrypt_sbox_xor(ct, k)
    assert rt == pt, f"Round-trip failed for k={k}, pt={pt!r} -> {rt!r}"


def run():
    # --- 1) Key limits: k = 0 and k = 255 ---
    for k in (0, 255):
        # Lengths that hit padding and block boundaries
        for n in (0, 1, 15, 16, 17, 31, 32):
            pt = ("A" * n)  # does not contain '\x00'
            assert_roundtrip(pt, k)

        # Plaintext with extreme byte values inside (0x00 and 0xFF), but NOT at the end
        # so zero-unpadding will not truncate the message.
        pt = "\x00" + "HELLO" + "\xff" + "X"
        assert_roundtrip(pt, k)

        # Plaintext with many high latin-1 bytes (1..255). This includes 0xFF but excludes 0x00.
        pt = "".join(chr(i) for i in range(1, 256))
        assert_roundtrip(pt, k)

    # --- 2) Check that ciphertext length is always a multiple of 16 ---
    for k in (0, 255):
        for pt in ("", "A", "A" * 16, "A" * 17):
            ct = encrypt_sbox_xor(pt, k)
            assert len(ct) % 16 == 0, "Ciphertext length is not a multiple of 16"

    # --- 3) Out-of-range key must be rejected (k = 300) ---
    try:
        encrypt_sbox_xor("test", 300)
        raise AssertionError("Expected ValueError for k=300 in encrypt, but it did not raise.")
    except ValueError:
        pass

    try:
        decrypt_sbox_xor(b"\x00" * 16, 300)
        raise AssertionError("Expected ValueError for k=300 in decrypt, but it did not raise.")
    except ValueError:
        pass

    # --- 4) Explicit test of the zero-padding limitation (demonstration only) ---
    # This is NOT expected to round-trip, because decrypt() uses rstrip(b'\x00'),
    # which removes trailing zero bytes from the plaintext.
    for k in (0, 255):
        pt = "ABC\x00"  # ends with '\x00'
        ct = encrypt_sbox_xor(pt, k)
        rt = decrypt_sbox_xor(ct, k)
        assert rt == "ABC", f"Expected trailing zero to be stripped for k={k}, got {rt!r}"
